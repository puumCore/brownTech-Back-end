...
public class Controller extends Brain {
    ...
    public static Thread logoutThread;
    ...
    void allow_user_to_login(ActionEvent event) {
        ...
        final AccountObj accountObj = get_user_account(usernameTF.getText().trim(), password);
        if (accountObj == null) {
            error_message("Invalid credentials!", "Check your username or password").show();
        } else if (!accountObj.isAdmin()) {
            error_message("Failed!", "You lack the credentials needed to use this application").show();
        } else {
            Brain.myAccountObj = accountObj;
            logoutThread = new Thread(logout_from_elseWhere());
    ...
    void reset_user_password(ActionEvent event) {
        if (event != null) {
            ...
            AccountObj accountObj = get_user_account(usernameTF.getText().trim(), null);
            if (accountObj == null) {
                error_message("Bad username!", "Try to remember your username to continue...").show();
            } else {
                final String GENERATED_PLAIN_PASSWORD = RandomStringUtils.randomAlphabetic(10);
                accountObj.setPassword(GENERATED_PLAIN_PASSWORD);
                accountObj = reset_password(accountObj);
                if (accountObj == null) {
                    error_message("Failed!", "Your password was not updated, please retry").show();
                } else {
                    success_notification("Your accountObj password has been updated!").show();
                }
            }
        }
    }
